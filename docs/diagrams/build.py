...
def overview() -> None:
    ...
    d.box("prs", 300, 220, 260, 60, "Normalizer → RuleParser (+LLM fallback)<br>→ ObservationBuilder<br><font style='font-size:10px'>текст → факти → Observation</font>", color="worker")
    d.box("cor", 300, 300, 260, 60, "Deduplicator → Correlator<br><font style='font-size:10px'>Observation → ThreatTrack + Revision</font>", color="worker")
    ...
    d.box("db", 640, 120, 220, 300, "PostgreSQL + PostGIS<br><br>raw_messages<br>observations<br>threat_tracks<br>threat_track_revisions<br>air_alerts<br>places · таксономія · sources<br><br><font style='font-size:10px'>єдине джерело істини</font>", DB, "db")
    ...
    d.edge("prs", "db", "Observation", exit=(1, 0.5), entry=(0, 0.45))
...
def pipeline() -> None:
    ...
    d.box("alert", 60, 115, 240, 60, "AlertsInUaHandler<br><font style='font-size:10px'>AirAlert + Observation(AirRaidAlert/AlertCancelled), без NLP</font>", color="worker")
    ...
    d.box("bld", x, 500, w, 70, "ObservationBuilder<br><font style='font-size:10px'>рівень класифікації + впевненість (обмежена довірою джерела); район ≠ точка; напрямок; metadata «чому»</font>", color="worker")
    d.box("obs", x, 600, w, 50, "Observation", color="db")
    ...
    d.box("trk", x, 870, w, 60, "ThreatTrack + ThreatTrackRevision<br><font style='font-size:10px'>ревізія = знімок для історії (час події)</font>", color="db")
    ...
    d.note(760, 500, 260, 90, "Кожне повідомлення може дати 0..N observations (по одному на сегмент/загрозу). Повідомлення без фактів — теж результат (метрика parser.unmatched).")
...
def datamodel() -> None:
    ...
    table("sources", 40, 40, 220, "sources", ["code, name, type", "trust_level 0..1, priority", "config jsonb (channel, …)"], "src")
    ...
    table("obs", 340, 120, 260, "observations", ["raw_message_id, source_id, segment_text", "event_type, observed_at", "category / class / family / model", "model_/classification_/observation_confidence", "location_kind, location (centroid), accuracy_km", "location_place_id, origin_/destination_place_id", "direction_deg, direction_kind, direction_confidence", "duplicate_of_observation_id", "parser_metadata jsonb (правила, спани)"], "db")
    table("tto", 680, 200, 220, "threat_track_observations", ["threat_track_id, observation_id", "sequence", "association_confidence 0..1", "association_reason jsonb"], "db")
    table("trk", 980, 120, 240, "threat_tracks", ["status, closed_reason", "category / class / family / model", "first_seen_at, last_seen_at, updated_at", "last_location*, track_geometry", "direction_*, object_count", "track_confidence, observation_count", "distinct_source_count, last_source_id"], "db")
    table("rev", 980, 340, 240, "threat_track_revisions", ["threat_track_id, revision_at ★", "повна копія стану треку", "observation_id (тригер)"], "db")
    ...
    table("tax", 680, 520, 220, "таксономія", ["threat_categories → classes", "→ families → models", "metadata: speed, fade, window", "threat_model_aliases (стеми)"], "grey")
    ...
    d.note(40, 470, 260, 90, "★ — ключ ідемпотентності / replay.<br>Ланцюжок походження: карта → track → track_observations → observation → raw_message → source → URL оригіналу. Нічого не видаляється; дублікати лишаються.")
    d.write()


# ---- 4. correlation decision flow ---------------------------------------------------------------------------
def correlation() -> None:
    d = D("04-correlation", "Кореляція нового Observation")
    x, w = 420, 300
    d.box("in", x, 30, w, 50, "Observation (ThreatObserved, з категорією)", color="db")
    ...
    d.box("cand", x, 230, w, 60, "Кандидати: активні треки тієї ж категорії,<br>сумісний клас, last_seen ±2 год", color="worker")
    d.box("score", x, 320, w, 110, "Скоринг кожного кандидата<br><font style='font-size:10px'>0.30·час (вікно класу) + 0.35·простір (швидкість×Δt + радіуси районів + 30 км)<br>+ 0.15·напрямок + 0.20·клас/модель<br>= 0 якщо стрибок неможливий або те саме джерело за &lt; 8 хв назвало інший район</font>", color="worker")
    d.box("thr", x + 60, 470, 180, 80, "найкращий ≥ 0.6 ?", DIAMOND, "grey")
    d.box("new", 40, 480, 300, 60, "Новий ThreatTrack<br><font style='font-size:10px'>association_confidence = 1</font>", color="worker")
    ...
    d.box("conf", x, 700, w, 60, "track_confidence = найкраща observation<br>+1 за ≥2 джерела, +1 за ≥3 повідомлення", color="worker")
    ...
    d.note(800, 420, 280, 80, "Вікна класів: БпЛА 60 хв, крилаті 25 хв, балістика 10 хв (ThreatClass.Metadata, змінюється в БД).")
