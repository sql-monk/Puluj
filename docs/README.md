...
| **Puluj.Admin** (:5258) | адмін-панель окремим сервісом: усі налаштування (джерела з токенами, alerts.in.ua, Telegram, LLM, кореляція), рейтинг джерел, стан кожного компонента, статистика колекторів/обробки/БД і хвости логів. Роль `puluj_admin` (читання-запис таблиць, без DDL); у Docker — керування контейнерами стеку через прокинутий сокет (`DockerService`: масштабування `processor`, restart/stop/start будь-якого сервісу, крім `admin`/`postgis`/`migrate`, які захищені на сервері; лише контейнери з міткою `com.docker.compose.project=puluj`, кожна дія в лозі) |
...
| `Docker__{Enabled,Command,ComposeDir,Project,ComposeFiles,ScalableService,MaxReplicas,ProtectedServices}` (сервіс admin) | false, `docker`, `/deploy`, `puluj`, порожньо (= `docker-compose.yml` + `docker-compose.override.yml`, якщо є), `processor`, 8, `admin,postgis,migrate` | керування контейнерами з панелі; compose ставить `Enabled=true` і монтує `/var/run/docker.sock` та `./:/deploy:ro`. Поза Docker (`dev-run.ps1`) — «керування недоступне», решта панелі працює. **Безпека:** хто контролює сокет — контролює хост; панель обмежує себе контейнерами проєкту, сокет — ні, тому порт 8081 має лишатися приватним і `Admin__Token` заданим; щоб відключити — прибрати рядок із сокетом і `Docker__Enabled` у compose |
...
одночасно. Панель показує це в розділі **«Аналітика»** двома пунктами меню: «Хто кого копіює» (`#/analytics`) і «Стан сервісу»
(`#/analytics-service`) — один компонент, який за хешем відкриває потрібну вкладку. Вкладки: **Огляд** (KPI періоду —
пости, копії, частка унікальних, медіанна затримка копії; пости й копії за днями; теплокарта «копіювальник ×
першоджерело», клік по клітинці відкриває пару), **Джерела** (картка на джерело: пости/копії/скопійовано за днями,
унікальність, затримка сер./мед., випередження, години доби, пересилання внутрішні/зовнішні; сортування; окремо —
канали поза системою, з яких пересилають найчастіше), **Пари** (таблиця з фільтрами за копіювальником і
першоджерелом; клік по рядку — гістограма затримок за бакетами <1 / 1–5 / 5–15 / 15–60 хв / 1–6 год / >6 год і 20
останніх прикладів з текстами), **Першоджерела** (хто перший відкриває треки за категоріями, частки й відставання),
**Знахідки** (останні пари з обома текстами, фільтри за видом near/verbatim/forward, джерелом і «лише первинні»;
спільні слова від 4 літер підсвічено на клієнті), **Сервіс** (heartbeat, версія/час збірки/uptime/процес із
`Runtime:Worker:analytics:Status`, курсор і відставання від `raw_messages`, останні прогони з тривалістю і швидкістю
msg/s, розмір схеми, міграції; «Перебудувати індекс» із підтвердженням очищає індекс — сервіс проходить усі
повідомлення заново з поточними порогами, ~400 тис. — близько 20 хв). Період — 7/14/30 днів (Київ). Поки схеми
`analytics` немає, панель показує порожній стан із підказкою запустити сервіс. Пороги — `Analytics__*` у конфігурації
сервісу; сам сервіс віддає `GET /health` (503 після двох невдалих прогонів поспіль) і
`GET /api/analytics/{status,report,recent}`.
...
| `GET /api/stats/targets?from&to` | вкладка «Цілі»: факти (без повторів між джерелами) і треки за бакет і категорією, класи, області (топ-15 + «інші»), маршрути між областями, година × день тижня |
| `GET /api/stats/alerts?from&to` | вкладка «Тривоги»: тривоги рівня області/Київ — оголошені та години під тривогою за бакет (інтервали обрізані періодом і розрізані по бакетах), по областях, гістограма тривалості, за годиною доби, топ днів |
| `GET /api/stats/sources?from&to` | вкладка «Джерела»: на джерело — повідомлень, оброблено, з фактами, фактів, медіанна затримка (< 6 год), ряд за бакетами |
| `GET /api/stats/recognition?from&to` | вкладка «Розпізнавання»: зрізи типів подій, методу, впевненості, точності локації; оброблено / з фактами за бакет. Спільне для чотирьох: без параметрів — 24 год; кінець обрізається до «зараз», довжина 1 год … 366 днів; крок `hour`/`day`/`week` (≤ 3 дні / ≤ 120 днів / далі), бакети в `Europe/Kyiv`; кеш 2 хв на `section\|from\|to` (`IMemoryCache`, Size = 1); `Cache-Control: max-age=60`; лише агрегати — нічого про глядача |
...
| `/api/admin/ops/*` | `overview` (стан сервісів + БД + `processorCount`), `collectors`, `db`, `workers` (інстанси: heartbeat + статус + оброблено за 24 год / у роботі + контейнер), `containers` (контейнери проєкту зі станом/CPU/пам'яттю, доступність Docker, кількість реплік `processor`), `POST containers/{id}/{restart\|stop\|start}` (403 для `admin`/`postgis`/`migrate`, 404 — не з проєкту, 502 — помилка docker, 503 — Docker недоступний), `POST processors/scale` `{replicas}` (`docker compose up --scale processor=N`, 0…8; відповідь `{result, containers}`), `pipeline?hours=24\|168\|720` (звіт конвеєра, замінив `processing`), `POST reprocess`; `/api/admin/logs/files`, `/api/admin/logs?file=&lines=&filter=&level=` |
| `/api/admin/analytics/*` | `status` (курсор, відставання, прогони, розмір схеми, міграції, `instance` з `Runtime:Worker:analytics:Status` або null), `report?days=` (джерела, пари, зовнішні пересилання, перші по треках; 404 до першого прогону), `recent?limit=&sourceId=&kind=near|verbatim|forward&primaryOnly=` (пари з текстами), `pairs/{copierId}/{originalId}?days=` (одна пара: агрегати, гістограма затримок `delays[]` за бакетами 1/5/15/60/360 хв, 20 останніх прикладів), `POST reset` |
...
Публічна сторінка `#/stats` — чотири вкладки, кожна відповідає на одне питання і вантажить лише свій payload:
**Цілі** (що летіло: факти/обʼєкти за категоріями у часі, класи, області, маршрути «звідки → куди», година × день
тижня), **Тривоги** (години під тривогою й оголошені за бакет, по областях, тривалість, година доби, найважчі дні),
**Джерела** (таблиця: повідомлень, оброблено, з фактами, фактів, медіанна затримка, спарклайн; повідомлення за
джерелами у часі), **Розпізнавання** (типи подій, метод, впевненість, точність локації, частка повідомлень без
фактів). Період спільний: 24 год / 7 д / 30 д / 90 д / довільний, у хеші (`#/stats?tab=alerts&p=7d`, `?from&to`),
кінець округлюється до 5 хв. Живий рядок угорі — з того, що store тримає для карти (треки, тривоги, стрічка), без
запиту. Графіки — власні SVG (`web/src/stats/charts/`): осі, сітка, одна підказка на бакет, стрілки ←/→,
`aria-label`, кнопка «таблиця» під кожним, кольори з теми через CSS-змінні. Бекенд: `Puluj.Api/Services/StatsService.cs`
(SQL `GROUP BY`), `StatsBuckets.cs` (період і бакети, `Europe/Kyiv`), `StatsFolds.cs` (категорії, області, зрізи),
`AlertIntervals.cs` (чиста арифметика інтервалів тривог; `tests/Puluj.Api.Tests`). Ендпоїнти — у таблиці [API](#api).
...
**Стан інстансів.** Кожен процес Worker (і сервіс аналітики) раз на 10 с пише в `app_settings` ключ
`Runtime:Worker:{InstanceName}:Status` — JSON `WorkerStatusDto` (`Puluj.Contracts/OpsDtos.cs`): версія збірки та час
запису dll (`builtAt` — саме він відрізняє репліки різних збірок), ролі, PID/CPU/RSS/потоки, а для інстансів із роллю
`processing` — лічильники за результатами, повідомлень/хв за 1 і 5 хв, таймінги parse/lock/store/total (p50/p90/max за
останні 5 хв, буфер 2 000 вимірів), поточні claim-и та стан LLM-запобіжника (пауза до, причина, викликів/збоїв).
Heartbeat-ключ `…:Heartbeat` лишається сигналом «живий»; обидва ключі видаляються при чистій зупинці. Панель показує
це в розділі «Воркери». Час успішної обробки повідомлення (parse + очікування store-lock + store, без коміту)
зберігається в `raw_messages.processing_ms` (міграція `AddRawMessageProcessingMs`; для Skipped/Failed — NULL), тож
p50/p90 за джерелом, годиною чи інстансом рахуються в SQL через `percentile_cont` (розділ «Конвеєр»).

**Воркери** (`#/workers`, опитування 5 с): картка на інстанс — версія і час збірки dll (відрізняє репліки різних
збірок), uptime, ролі, PID/CPU/RSS; для процесорів — воркерів, оброблено за 24 год (з БД за `claimed_by`),
повідомлень/хв, таймінги parse/lock/store/total (p50/p90 за 5 хв), що зараз у роботі, стан LLM; контейнер із кнопками
«Перезапустити»/«Зупинити»/«Запустити» (з підтвердженням і наслідком; для `admin`/`postgis`/`migrate` — «керується
лише з консолі»). Блок «Процесори повідомлень» — поле кількості реплік (0…8; 0 = обробка зупиняється, панель
попереджає) і результат compose. Репліка, зупинена кнопкою, лишає claim-и — їх повертає ліз (5 хв) або чистий SIGTERM
(`docker stop` дає 10 с). Якщо замість таблиці контейнерів текст «docker недоступний» — перевірити монтування сокета:
`docker compose exec admin docker version`.

**Конвеєр** (`#/pipeline`, 24 год / 7 д / 30 д, час Europe/Kyiv): прийнято / оброблено / з фактами / цілей / треків /
у черзі / помилок, p50/p90/середнє з `raw_messages.processing_ms` (лише успішні обробки; старі рядки — NULL,
показуються «—»), графік за бакетами з лінією p90, таблиці за джерелами (затримка = медіана received − published,
< 6 год; треки — за першою ціллю) і за інстансами, помилки за етапами і останні 50.
