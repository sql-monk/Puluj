...
/// errors and source statistics, and puts every raw message back to Pending. The Worker's sweeper then re-runs the
...
public sealed class ReprocessService(IDbContextFactory<PulujDbContext> factory, SettingsStore settings, ILogger<ReprocessService> logger)
{
    ...
    public async Task<int> ResetAsync(CancellationToken ct)
    {
        ...
        // Failed ones too: a parser fix is one of the reasons to reprocess.
        await db.Database.ExecuteSqlRawAsync("UPDATE raw_messages SET processing_status = 0, attempts = 0, processed_at = NULL WHERE processing_status <> 0", ct);
    ...
    /// <summary>Holds the sweeper: pending messages stay in the database until <see cref="ResumeAsync"/>.</summary>
