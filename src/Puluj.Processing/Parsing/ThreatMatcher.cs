...
/// <summary>Finds threat aliases in a segment; overlapping matches resolve to the longest, then highest priority.</summary>
public sealed class ThreatMatcher(TaxonomyIndex taxonomy)
{
    ...
    public IReadOnlyList<ThreatMention> Match(Segment segment, ParseContext ctx)
    {
        ...
        foreach (var m in found.OrderByDescending(f => f.Len).ThenByDescending(f => f.MatchedChars).ThenByDescending(f => f.Alias.Priority))
        {
            ...
            var threatRef = taxonomy.Resolve(m.Alias.Level, m.Alias.TargetId);
            if (threatRef is null)
            {
                continue;
            }
            result.Add(new ThreatMention(threatRef, segment.Slice(m.Start, m.Start + m.Len), m.Alias.ImpliedConfidence, hedged, m.Start, m.Len));
        }
